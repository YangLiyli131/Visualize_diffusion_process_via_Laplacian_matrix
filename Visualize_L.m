...
for t = 0:0.05:5
   ...
   if t == 0
      imwrite(imind, cm, 'out.gif', 'gif', 'Loopcount', inf, 'DelayTime', 0.5); 
   else
      imwrite(imind, cm, 'out.gif', 'gif', 'WriteMode', 'append', 'DelayTime', 0.5);
   end
end
...
pr1 = AA;
pr2 = inv(DA) * AA;
pr3 = mpower(DA,-0.5) * AA * mpower(DA,-0.5);
...
for t = 1:50
   ...
   bar3(Phi);
   caxis([0, 10]);
   title(strcat('number of hops = ', num2str(t)));

   frame = getframe(1);
   im = frame2im(frame);
   [imind, cm] = rgb2ind(im, 256);
   if t == 1
      imwrite(imind, cm, 'out.gif', 'gif', 'Loopcount', inf, 'DelayTime', 1); 
   else
      imwrite(imind, cm, 'out.gif', 'gif', 'WriteMode', 'append', 'DelayTime', 1);
   end
